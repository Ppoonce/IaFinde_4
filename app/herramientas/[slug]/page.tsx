      ...
      ? [
          {
            ...
            a: `Las alternativas más comparables son ${alternatives.map((a) => a.name).join(', ')}. Puedes ver el detalle de cada una en la sección de herramientas similares, más abajo en esta misma página.`,
          },
        ]
      : []),
  ];
  ...
  return (
    <div className="pb-20">
      ...
      <article className="mx-auto max-w-4xl px-5 pt-6">
        <p className="rounded-full bg-paper px-3 py-1 text-xs font-medium text-inkmuted inline-block border border-line">
          Análisis editorial independiente de IAFinder · Actualizado {tool.lastUpdated}
        </p>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          ...
          <span className="rounded-full border border-line bg-surface px-2.5 py-0.5 text-xs font-medium text-ink">
            ★ {tool.editorialScore.toFixed(1)}/5 · Valoración editorial
          </span>
          {tool.isNew && (
        ...
        <section className="mt-10">
          <h2 className="font-display text-xl font-semibold text-ink">Cómo empezar con {tool.name}</h2>
          <ol className="mt-4 space-y-3">
            <li className="flex gap-3 text-sm text-inkmuted">
              <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-ink text-xs font-semibold text-white">
                1
              </span>
              <span>
                Entra en el sitio oficial de {tool.name} y crea una cuenta{tool.freeVersion ? ' con el plan gratuito' : tool.freeTrial ? ' con la prueba gratuita disponible' : ''}.
              </span>
            </li>
            <li className="flex gap-3 text-sm text-inkmuted">
              <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-ink text-xs font-semibold text-white">
              ...
              <span>Explora las funciones principales descritas arriba antes de decidir si necesitas un plan de pago.</span>
            </li>
            <li className="flex gap-3 text-sm text-inkmuted">
              <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-ink text-xs font-semibold text-white">
                3
              ...
              <span>
                Si no termina de encajar, revisa las herramientas similares al final de esta página antes de comprometerte
                con un plan de pago.
              </span>
            </li>
          </ol>
