  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <button
          type="submit"
          className="press-feedback rounded-xl bg-signal px-6 py-3 text-sm font-medium text-white transition-colors hover:bg-signaldark"
        >
          Buscar herramientas
        </button>
      </form>
        ...
        <div className="absolute z-20 mt-2 w-full rounded-xl border border-line bg-surface p-2 shadow-[0_8px_24px_rgba(21,23,28,0.10)]">
          ...
          {suggestions.map((s) => (
            <button
              ...
              className="press-feedback block w-full rounded-lg px-3 py-2 text-left text-sm text-ink hover:bg-paper"
            >
              {s.label}
            </button>
          ))}
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {examples.map((ex) => (
          <button
            ...
            className="press-feedback rounded-full border border-line bg-surface px-3.5 py-1.5 text-sm text-inkmuted transition-colors hover:border-signal hover:text-signal"
          >
            {ex}
          </button>
        ))}
